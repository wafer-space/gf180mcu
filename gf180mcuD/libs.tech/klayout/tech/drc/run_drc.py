...
def get_list_of_tables(drc_dir: str, switches: dict):
    ...
    if switches["feol"] == "true":
        add_tables(Path(drc_dir) / "rule_decks" / "feol", tables)

    if switches["beol"] == "true":
        add_tables(Path(drc_dir) / "rule_decks" / "beol", tables)

    if switches["offgrid"] == "true":
        add_tables(Path(drc_dir) / "rule_decks" / "geom", tables)
...
def generate_klayout_switches(arguments, layout_path: str) -> dict:
    ...
    switches["feol"] = "false" if arguments.no_feol else "true"
    switches["beol"] = "false" if arguments.no_beol else "true"
    switches["offgrid"] = "false" if arguments.no_offgrid else "true"
    switches["conn_drc"] = "false" if arguments.no_connectivity else "true"
    switches["density"] = "false" if arguments.no_density else "true"
    switches["slow_via"] = "true" if arguments.slow_via else "false"
...
def check_klayout_version():
    ...
    if (major, minor, patch) < (0, 28, 4):
        logging.error("Minimum required KLayout version is 0.28.4.")
...
def run_check(
    ...
    logging.info(
        f"Running GF180MCU {' '.join(drc_tables)} checks on design {layout_path}, topcell: {topcell}"
    )
    ...
    logging.info(f"Running KLayout: {run_cmd}")
...
def run_parallel_run(
    ...
    for table in table_list:
        rule_deck_files[table] = rule_deck_full_path / "gf180mcu.drc"
...
def run_single_processor(
    ...
    if "main" not in tables:
        ...
        switches["beol"] = "false"
        switches["offgrid"] = "false"
    result_dbs.append(run_check(rule_deck_full_path / "gf180mcu.drc", tables, layout_path, run_dir, switches))
...
def parse_args():
    ...
    parser = argparse.ArgumentParser(
        description="Run GF180MCU OpenSource DRC checks",
        usage=USAGE,
    ...
    parser.add_argument(
        ...
        choices=["A", "B", "C", "D", "E", "F"],
        default="D",
        help="""R|Select combined options of metal_top, mim_option, and metal_level.
...
variant=F: Select  metal_top=9K   mim_option=A  metal_level=6LM""",
    ...
    parser.add_argument(
        "--verbose",
        action="store_true",
        help="Enable verbose mode in KLayout DRC.",
    )
    parser.add_argument(
        "--no_connectivity",
        action="store_true",
        help="Turn off connectivity rules.",
    )
    parser.add_argument(
        "--slow_via",
        action="store_true",
        help="Turn on SLOW_VIA option for MT30.8 rule.",
    )
